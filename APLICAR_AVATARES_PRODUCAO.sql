-- ═══════════════════════════════════════════════════════════════
-- SCRIPT PARA APLICAR AVATARES EM PRODUÇÃO
-- Execute este script no Supabase Studio SQL Editor
-- ═══════════════════════════════════════════════════════════════

-- IMPORTANTE: Este script atribui avatares de forma balanceada
-- a todos os posts e comentários que ainda não possuem avatar.

-- ═══════════════════════════════════════════════════════════════
-- PASSO 1: Criar tabela temporária com os 30 avatares
-- ═══════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════
-- PASSO 2: Atualizar POSTS sem avatar
-- ═══════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════
-- PASSO 3: Atualizar COMENTÁRIOS sem avatar
-- ═══════════════════════════════════════════════════════════════
...
SELECT
  COUNT(*) FILTER (WHERE "avatarId" IS NOT NULL) as posts_com_avatar,
  COUNT(*) FILTER (WHERE "avatarId" IS NULL) as posts_sem_avatar,
  COUNT(*) as total_posts
FROM "Post";

-- Contar comentários com avatar
SELECT
  COUNT(*) FILTER (WHERE "avatarId" IS NOT NULL) as comments_com_avatar,
  COUNT(*) FILTER (WHERE "avatarId" IS NULL) as comments_sem_avatar,
  COUNT(*) as total_comments
FROM "Comment";

-- Distribuição de avatares em posts
SELECT
  "avatarId",
  COUNT(*) as quantidade,
  ROUND(COUNT(*) * 100.0 / (SELECT COUNT(*) FROM "Post"), 2) as percentual
FROM "Post"
...
-- RESULTADO ESPERADO:
-- ✅ Todos os posts agora devem ter avatarId, avatarImg e avatarInitialsColor
-- ✅ Todos os comentários agora devem ter avatarId, avatarImg e avatarInitialsColor
-- ✅ Distribuição balanceada entre os 30 avatares disponíveis
