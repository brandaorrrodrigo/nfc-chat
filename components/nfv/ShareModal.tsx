 ...
 * Gera card visual (Canvas) e permite compartilhar via WhatsApp, Instagram, download
 */
...
function extractShareData(
  ...
  const score = (displayAnalysis.score as number) ||
                (displayAnalysis.overall_score as number) || 0;

  const report = (displayAnalysis.report as Record<string, unknown>) || {};
  const classificacao = (report.classificacao as string) ||
                        (displayAnalysis.classificacao as string) ||
                        (displayAnalysis.classification as string) || '';

  // Pontos criticos — novo formato
  const pontosCriticosNovo = (displayAnalysis.pontos_criticos as Array<{
    nome: string;
    severidade: string;
  }>) || [];

  // Pontos criticos — antigo formato (fallback)
  const pontosCriticosAntigo = ((report.pontos_criticos as Array<{
    tipo: string;
    descricao: string;
    severidade: string;
  }>) || []).map(p => ({ nome: p.descricao || p.tipo, severidade: p.severidade }));

  const pontosCriticos = pontosCriticosNovo.length > 0 ? pontosCriticosNovo : pontosCriticosAntigo;
  ...
  return {
    exerciseName: PATTERN_LABELS[analysis.movement_pattern] || analysis.movement_pattern,
...
export default function ShareModal({ analysis, displayAnalysis, open, onClose }: ShareModalProps) {
  ...
  useEffect(() => {
    if (open && canvasRef.current) {
      renderShareCardPreview(canvasRef.current, shareData);
    }
  }, [open, shareData]);

  if (!open) return null;

  const analysisUrl = `https://chat.nutrifitcoach.com.br/comunidades/${
    NFV_CONFIG.PREMIUM_ARENAS.find(a => a.pattern === analysis.movement_pattern)?.slug || 'analise'
  }/videos/${analysis.id}`;

  const whatsappText = `🏋️ Meu ${shareData.exerciseName.toLowerCase()} marcou ${shareData.score.toFixed(1)}/10 na analise biomecanica por IA! Confira sua tecnica tambem: chat.nutrifitcoach.com.br`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-zinc-900 border border-zinc-700 rounded-2xl max-w-md w-full p-6 max-h-[90vh] overflow-y-auto">
        ...
        {/* Canvas Preview */}
        <div className="rounded-xl overflow-hidden border border-zinc-700 mb-5">
          <canvas
            ref={canvasRef}
            className="w-full"
