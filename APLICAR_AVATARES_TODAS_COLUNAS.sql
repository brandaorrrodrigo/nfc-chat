-- ═══════════════════════════════════════════════════════════════
-- SCRIPT: ADICIONAR TODAS AS COLUNAS POSSÍVEIS PARA TRIGGERS
-- ═══════════════════════════════════════════════════════════════

-- PASSO 1: Adicionar TODAS as variações de timestamp (para qualquer trigger)
DO $$
BEGIN
    -- Post: Todas as variações possíveis
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'Post' AND column_name = 'updatedAt') THEN
    ...
    -- Comment: Todas as variações possíveis
    ...
    RAISE NOTICE '✓ Todas as colunas adicionadas';
END $$;
...
WITH posts_to_update AS (
  SELECT id FROM "Post" WHERE "avatarId" IS NULL ORDER BY "createdAt"
),
avatar_assignment AS (
  SELECT p.id, a.id as avatar_id, a.img as avatar_img, a.color as avatar_color
  FROM posts_to_update p
  CROSS JOIN LATERAL (SELECT id, img, color FROM avatar_pool ORDER BY random() LIMIT 1) a
)
UPDATE "Post" SET "avatarId" = aa.avatar_id, "avatarImg" = aa.avatar_img, "avatarInitialsColor" = aa.avatar_color
FROM avatar_assignment aa WHERE "Post".id = aa.id;

-- PASSO 5: Atualizar comentários
WITH comments_to_update AS (
  SELECT id FROM "Comment" WHERE "avatarId" IS NULL ORDER BY "createdAt"
),
avatar_assignment AS (
  SELECT c.id, a.id as avatar_id, a.img as avatar_img, a.color as avatar_color
  FROM comments_to_update c
  CROSS JOIN LATERAL (SELECT id, img, color FROM avatar_pool ORDER BY random() LIMIT 1) a
)
UPDATE "Comment" SET "avatarId" = aa.avatar_id, "avatarImg" = aa.avatar_img, "avatarInitialsColor" = aa.avatar_color
FROM avatar_assignment aa WHERE "Comment".id = aa.id;

-- PASSO 6: Resultados
SELECT 'POSTS' as tabela, COUNT(*) FILTER (WHERE "avatarId" IS NOT NULL) as com_avatar, COUNT(*) as total FROM "Post"
UNION ALL
SELECT 'COMMENTS', COUNT(*) FILTER (WHERE "avatarId" IS NOT NULL), COUNT(*) FROM "Comment";

-- Top 10
SELECT "avatarId", COUNT(*) as qtd FROM "Post" WHERE "avatarId" IS NOT NULL GROUP BY "avatarId" ORDER BY qtd DESC LIMIT 10;
