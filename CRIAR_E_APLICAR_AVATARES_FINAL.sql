-- ═══════════════════════════════════════════════════════════════
-- SCRIPT COMPLETO: CRIAR COLUNAS E APLICAR AVATARES
...
-- PASSO 2: Adicionar coluna updatedAt se não existir (necessário para triggers)
DO $$
BEGIN
    -- Post
    IF NOT EXISTS (
...
-- PASSO 3: Adicionar colunas de avatar na tabela Post
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns
...
-- PASSO 4: Adicionar colunas de avatar na tabela Comment
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns
...
-- PASSO 6: Criar tabela temporária com os 30 avatares
CREATE TEMP TABLE IF NOT EXISTS avatar_pool AS
SELECT * FROM (VALUES
...
-- PASSO 7: Desabilitar triggers e atualizar posts
DO $$
DECLARE
    posts_atualizados INTEGER;
BEGIN
    -- Desabilitar triggers
    ALTER TABLE "Post" DISABLE TRIGGER ALL;

    -- Atualizar posts sem avatar
    ...
    GET DIAGNOSTICS posts_atualizados = ROW_COUNT;

    -- Reabilitar triggers
    ALTER TABLE "Post" ENABLE TRIGGER ALL;

    RAISE NOTICE '✓ % posts atualizados', posts_atualizados;
END $$;

-- PASSO 8: Desabilitar triggers e atualizar comentários
DO $$
DECLARE
    comments_atualizados INTEGER;
BEGIN
    -- Desabilitar triggers
    ALTER TABLE "Comment" DISABLE TRIGGER ALL;

    -- Atualizar comentários sem avatar
    ...
    GET DIAGNOSTICS comments_atualizados = ROW_COUNT;

    -- Reabilitar triggers
    ALTER TABLE "Comment" ENABLE TRIGGER ALL;

    RAISE NOTICE '✓ % comentários atualizados', comments_atualizados;
END $$;

-- PASSO 9: Verificar resultados
SELECT
  'POSTS' as tabela,
