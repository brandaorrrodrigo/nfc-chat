/**
 * Seed script que usa .env.local para conectar ao Docker PostgreSQL
 * Carrega credenciais locais primeiro antes de importar Prisma
 */
...
// 1. Carregar .env.local ANTES de qualquer import que use DATABASE_URL
const envLocalPath = path.join(__dirname, '..', '.env.local');
if (fs.existsSync(envLocalPath)) {
  console.log('Loading .env.local for Docker credentials...');
  dotenv.config({ path: envLocalPath });
} else {
  console.log('WARNING: .env.local not found, using current environment');
}

// 2. Verificar credenciais foram carregadas
console.log('Database URL:', process.env.DATABASE_URL?.substring(0, 50) + '...');

// 3. Agora importar Prisma
import { PrismaClient } from '../lib/generated/prisma';

const prisma = new PrismaClient();

async function main() {
  console.log('\n=== Importando dados do JSON (Docker) ===\n');
