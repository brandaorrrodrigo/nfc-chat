/**
 * TESTE DA ARENA DOR MENSTRUAL
 *
 * Execução: npx tsx scripts/test-arena-menstrual.ts
 */
...
console.log('════════════════════════════════════════════════════════');
console.log('🧪 TESTE: ARENA DOR MENSTRUAL LIPEDEMA');
console.log('════════════════════════════════════════════════════════\n');
...
const threadPrincipal = gerarThread('menstrual');
...
if (temIA && respostaIA) {
  ...
  // Verificar se é uma resposta específica de menstrual
  const termosMenstrualEsperados = [
    'hormonal',
    'hormonios',
    'menstrual',
    'menstruacao',
  ...
  const contemTermoMenstrual = termosMenstrualEsperados.some(termo =>
    respostaIA.conteudo.toLowerCase().includes(termo)
  );

  if (contemTermoMenstrual) {
    console.log(`   ✅ Resposta específica de dor menstrual detectada!`);
  } else {
    console.log(`   ⚠️  Resposta genérica (não específica de dor menstrual)`);
  }
} else {
...
const threads = [];
for (let i = 0; i < 5; i++) {
  threads.push(gerarThread('menstrual'));
}
...
// ============================================
// TESTE 6: VERIFICAR TERMOS ESPECÍFICOS DE MENSTRUAL
// ============================================

console.log(`\n\n📝 TESTE 6: Termos Específicos de Dor Menstrual`);
console.log('─'.repeat(60));
...
if (termosEncontrados.length < 5) {
  console.log(`\n⚠️  ATENÇÃO: Poucos termos específicos de dor menstrual encontrados`);
} else {
...
console.log('   npx tsx scripts/populate-communities.ts lipedema-dor-menstrual\n');
