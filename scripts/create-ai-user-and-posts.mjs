...
async function main() {
  console.log('🏟️ CRIAR USUÁRIO IA E COMPLETAR SEED\n');

  try {
    // 1. Criar usuário IA com campos corretos
    console.log('👤 Criando usuário IA...');
    const { data: existingAiUser, error: checkError } = await supabase
      .from('User')
      .select('id')
      .eq('id', aiUserId)
      .maybeSingle();
    ...
    if (existingAiUser) {
      console.log('   ✅ Usuário IA já existe');
    } else {
      ...
      if (createError) {
        ...
        console.log('   ✅ Usuário IA criado com sucesso!');
      }
    }
    ...
    baseTime = new Date(baseTime.getTime() + (postCount || 0) * 70 * 60000);
    ...
    for (let i = 0; i < remainingPosts.length; i++) {
      ...
      if (postError) {
        console.error(`   ❌ Post ${i + 1}: ${postError.message}`);
        errorCount++;
      } else {
    ...
    console.log(`📊 Posts criados: ${successCount} ✅`);
    if (errorCount > 0) console.log(`❌ Posts com erro: ${errorCount}`);
    console.log(`\n📈 Total na arena: ${finalCount}`);
