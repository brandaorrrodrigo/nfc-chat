/**
 * TESTE DA ARENA BARRIGA POCHETE / POSTURA
 *
 * Execução: npx tsx scripts/test-arena-barriga.ts
 */
...
console.log('════════════════════════════════════════════════════════');
console.log('🧪 TESTE: ARENA BARRIGA POCHETE / POSTURA');
console.log('════════════════════════════════════════════════════════\n');
...
const threadPrincipal = gerarThread('postura');
...
if (validacaoThread.problemas.length > 0) {
  console.log(`   Problemas: ${validacaoThread.problemas.join(', ')}`);
}
...
if (temIA && respostaIA) {
  ...
  // Verificar se é uma resposta específica de postura
  const respostasPosturaEsperadas = [
    'anteversao pelvica',
    'transverso',
    'gluteo fraco',
    'lordose',
  ...
  const contemTermoPostura = respostasPosturaEsperadas.some(termo =>
    respostaIA.conteudo.toLowerCase().includes(termo)
  );

  if (contemTermoPostura) {
    console.log(`   ✅ Resposta específica de postura detectada!`);
  } else {
    console.log(`   ⚠️  Resposta genérica (não específica de postura)`);
  }
} else {
...
const threads = [];
for (let i = 0; i < 5; i++) {
  threads.push(gerarThread('postura'));
}
...
// ============================================
// TESTE 6: VERIFICAR TERMOS ESPECÍFICOS
// ============================================

console.log(`\n\n📝 TESTE 6: Termos Específicos de Postura`);
console.log('─'.repeat(60));
...
if (termosEncontrados.length < 5) {
  console.log(`\n⚠️  ATENÇÃO: Poucos termos específicos de postura encontrados`);
} else {
...
console.log(`${temIA ? '✅' : '⚠️ '} Teste 3: Intervenção da IA     - ${temIA ? 'OK (IA respondeu)' : 'OK (IA não respondeu - 40% de chance)'}`);
...
console.log('   npx tsx scripts/populate-communities.ts barriga-pochete-postura\n');
