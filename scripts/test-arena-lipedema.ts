/**
 * TESTE DA ARENA MUSCULAÇÃO LIPEDEMA
 *
 * Execução: npx tsx scripts/test-arena-lipedema.ts
 */
...
console.log('════════════════════════════════════════════════════════');
console.log('🧪 TESTE: ARENA MUSCULAÇÃO LIPEDEMA');
console.log('════════════════════════════════════════════════════════\n');
...
const threadPrincipal = gerarThread('lipedema');
...
if (temIA && respostaIA) {
  ...
  // Verificar se é uma resposta específica de lipedema
  const termosLipedemaEsperados = [
    'lipedema',
    'inflamacao',
    'drenagem',
    'musculacao terapeutica',
    'gluteo',
    'panturrilha',
    'treinar nao inflama',
    'treinar errado inflama',
    'carga',
    'circulacao',
  ];

  const contemTermoLipedema = termosLipedemaEsperados.some(termo =>
    respostaIA.conteudo.toLowerCase().includes(termo)
  );

  if (contemTermoLipedema) {
    console.log(`   ✅ Resposta específica de lipedema detectada!`);
  } else {
    console.log(`   ⚠️  Resposta genérica (não específica de lipedema)`);
  }
} else {
...
const threads = [];
for (let i = 0; i < 5; i++) {
  threads.push(gerarThread('lipedema'));
}
...
// ============================================
// TESTE 6: VERIFICAR TERMOS ESPECÍFICOS DE LIPEDEMA
// ============================================

console.log(`\n\n📝 TESTE 6: Termos Específicos de Lipedema`);
console.log('─'.repeat(60));
...
if (termosEncontrados.length < 5) {
  console.log(`\n⚠️  ATENÇÃO: Poucos termos específicos de lipedema encontrados`);
} else {
...
const respostasObrigatorias = [
  'impacto piorou',
  'quando parei de treinar',
  'carga alta sem controle',
];
...
console.log('   npx tsx scripts/populate-communities.ts musculacao-lipedema\n');
