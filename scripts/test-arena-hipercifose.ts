/**
 * TESTE DA ARENA HIPERCIFOSE DRENAGEM
 *
 * Execução: npx tsx scripts/test-arena-hipercifose.ts
 */
...
console.log('════════════════════════════════════════════════════════');
console.log('🧪 TESTE: ARENA HIPERCIFOSE DRENAGEM');
console.log('════════════════════════════════════════════════════════\n');
...
const threadPrincipal = gerarThread('hipercifose');
...
if (temIA && respostaIA) {
  ...
  // Verificar se é uma resposta específica de hipercifose
  const termosHipercifoseEsperados = [
    'hipercifose',
    'postura',
    'respiracao',
    'retorno venoso',
  ...
  const contemTermoHipercifose = termosHipercifoseEsperados.some(termo =>
    respostaIA.conteudo.toLowerCase().includes(termo)
  );

  if (contemTermoHipercifose) {
    console.log(`   ✅ Resposta específica de hipercifose detectada!`);
  } else {
    console.log(`   ⚠️  Resposta genérica (não específica de hipercifose)`);
  }
} else {
...
const threads = [];
for (let i = 0; i < 5; i++) {
  threads.push(gerarThread('hipercifose'));
}
...
// ============================================
// TESTE 6: VERIFICAR TERMOS ESPECÍFICOS DE HIPERCIFOSE
// ============================================

console.log(`\n\n📝 TESTE 6: Termos Específicos de Hipercifose`);
console.log('─'.repeat(60));
...
if (termosEncontrados.length < 5) {
  console.log(`\n⚠️  ATENÇÃO: Poucos termos específicos de hipercifose encontrados`);
} else {
...
console.log('   npx tsx scripts/populate-communities.ts hipercifose-drenagem\n');
