/**
 * GERADOR DE DADOS - ARENA HIPERCIFOSE
 */
...
console.log('════════════════════════════════════════════════════════');
console.log('🏗️  GERADOR - ARENA HIPERCIFOSE DRENAGEM');
console.log('════════════════════════════════════════════════════════\n');

const threads = gerarThreadsEmLote('hipercifose', 5, {
  incluirIA: true,
  dataInicio: new Date(),
});
...
threads.forEach((thread, index) => {
  const postId = `post_hipercifose_${index + 1}`;

  dados.threads.push({
    id: postId,
    arena_slug: 'hipercifose-drenagem',
  ...
  thread.respostas.forEach((resposta, respostaIndex) => {
    dados.mensagens.push({
      id: `msg_hipercifose_${index}_${respostaIndex}`,
      comunidade_slug: 'hipercifose-drenagem',
      post_id: postId,
...
fs.writeFileSync('arena-hipercifose-dados.json', JSON.stringify(dados, null, 2));
console.log('✅ Dados salvos em: arena-hipercifose-dados.json\n');
