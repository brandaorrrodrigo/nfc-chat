...
**IMPORTANTE:** Este SQL precisa ser executado para o sistema de IA Avançada funcionar completamente.
...
2. Clicar em **+ New Query** (ou **Nova Query**)
3. Nome sugerido: `IA Facilitadora - Interventions Tables`
...
```
D:\NUTRIFITCOACH_MASTER\nfc-comunidades\supabase\migrations\ia_interventions.sql
```
...
2. Clicar em **RUN** (ou **Executar**) no canto inferior direito
...
Após executar o SQL, o sistema está **100% funcional**.

**Testar agora:**
1. Ir para: https://chat.nutrifitcoach.com.br/comunidades/receitas-saudaveis
2. Postar a receita de teste do arquivo `TESTE_IA_RECEITAS.md`
3. Verificar se IA responde com follow-up question
...
**Solução:** Tudo certo, SQL já foi executado!

### Erro: "permission denied"
**Causa:** Usuário sem permissão de CREATE TABLE

**Solução:**
1. Verificar se está logado como owner do projeto
2. Ou executar como service_role

### Erro: "syntax error"
**Causa:** SQL copiado errado

**Solução:**
1. Copiar novamente do arquivo original
2. Garantir que copiou TUDO (incluindo linhas em branco)

---

## 📊 Verificar se Funcionou

### Query 1: Ver tabelas criadas
```sql
SELECT table_name
...
### Query 2: Ver funções criadas
```sql
SELECT routine_name
FROM information_schema.routines
WHERE routine_name LIKE '%ia_stats%';
```

**Resultado esperado:**
```
get_user_ia_stats_today
upsert_user_ia_stats
```
...
### `nfc_chat_ia_interventions`
Armazena cada vez que a IA intervém:
- Quando interveio
- Para qual usuário
- Qual foi a pergunta de follow-up
- Se o usuário respondeu

### `nfc_chat_ia_user_stats`
Stats diários de cada usuário:
- Quantas intervenções recebeu hoje
- Quantas perguntas ignorou
- Quantas respondeu
- Probabilidade ajustada atual

---

**Após executar SQL:** Sistema pronto para funcionar! 🚀
