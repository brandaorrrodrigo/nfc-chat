-- Script para criar tabelas de comunidades no Supabase (VERSÃO FINAL)
-- Execute isso no Supabase Studio > SQL Editor

-- Criar tabela de comunidades (usando TEXT para id, compatível com User existente)
...
-- Verificar se o usuário admin já existe, se não, criar
...
CREATE INDEX IF NOT EXISTS "Topico_autor_id_idx" ON "Topico"(autor_id);
CREATE INDEX IF NOT EXISTS "Resposta_topico_id_idx" ON "Resposta"(topico_id);
CREATE INDEX IF NOT EXISTS "Resposta_autor_id_idx" ON "Resposta"(autor_id);
CREATE INDEX IF NOT EXISTS "MembroComunidade_comunidade_id_idx" ON "MembroComunidade"(comunidade_id);
CREATE INDEX IF NOT EXISTS "MembroComunidade_usuario_id_idx" ON "MembroComunidade"(usuario_id);
